...
library(terra) # mapas raster. former raster package
...
```{r}
Ituango <- read_excel("data/Ituango_cut.xlsx", sheet = "Ituango_cut")

# filter and make uniques
CToperation_Ituango <- Ituango |> 
            # dplyr::rename(Deployment_id="Deployment ID") |> 
            dplyr::mutate(year=year(as_date(Photo_Date)))|> 
            dplyr::mutate(Deployment_id="Ituango") |> 
            distinct(Longitude, Latitude, camera_trap_start_date, camera_trap_end_date, year, cameratrap, Deployment_id, ID)

# Generamos la matríz de operación de las cámaras

camop_Ituango  <- cameraOperation(CTtable= CToperation_Ituango , # Tabla de operación
                         stationCol= "ID", # define la estación
                         setupCol= "camera_trap_start_date", #Columna fecha de colocación
                         retrievalCol= "camera_trap_end_date", #Columna fecha de retiro
                         #hasProblems= T, # Hubo fallos de cámaras
                         dateFormat= "%Y-%m-%d", #, # Formato de las fechas
                         cameraCol="camera_trap",
                         byCamera=TRUE)#,
                         #sessionCol= "year")
...
DetHist_list <- lapply(unique(Ituango$binomial), FUN = function(x) {
  detectionHistory(
    recordTable         = Ituango, # Tabla de registros
    camOp                = camop_Ituango, # Matriz de operación de cámaras
    stationCol           = "ID",
    speciesCol           = "binomial",
    recordDateTimeCol    = "Photo_Date",
